...
### M1 — Root redirect + 平台設定檔（commit 5e32275）
- `index.html`：根目錄 JS + meta redirect → engine/
- `netlify.toml`：publish=. + root 302 redirect + cache headers
- `vercel.json`：root redirect + asset cache headers
- `.nojekyll`：GitHub Pages 允許 JS module 路徑
- `.gitignore`：加入 dist/ vn-demo.zip 等 generated files

### M2 — pack_demo.py 打包腳本（commit 88cbd93）
- `python pack_demo.py` → dist/ (54 files, 9.7 MB)
- `python pack_demo.py --zip` → 額外 vn-demo.zip (9.5 MB)

### M3 — GitHub Actions workflow（commit e032ef1）
- `.github/workflows/pages.yml`：push main/master 自動打包部署
- 手動觸發：GitHub repo Actions 分頁 → Run workflow

### M4 — README 部署章節（本次 commit）
- README 新增三平台 step-by-step 快速部署說明
