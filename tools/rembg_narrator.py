"""
narrator.jpg 去背 + 6 表情色彩分級腳本
輸出：assets/characters/narrator/*.png (400x800 RGBA)
"""
import io, sys
...
SRC = Path(r'C:\Users\User\vn-engine\tools\vrm\narrator.jpg')
OUT = Path(r'C:\Users\User\vn-engine\assets\characters\narrator')
OUT.mkdir(parents=True, exist_ok=True)
W, H = 400, 800

# ── Step 1: Remove background ──────────────────────────────────────
print("Step 1: rembg 去背...")
result_bytes = remove(SRC.read_bytes())
img_rgba = Image.open(io.BytesIO(result_bytes)).convert('RGBA')
print(f"  去背完成：{img_rgba.size}")

# ── Step 2: Crop to subject + fit 400×800 ─────────────────────────
print("Step 2: 裁切 + 縮放至 400×800...")
...
# Scale to fit 400×800
scale  = min(W / sw, H / sh)
nw, nh = int(sw * scale), int(sh * scale)
scaled  = cropped.resize((nw, nh), Image.LANCZOS)

# Paste bottom-centred on 400×800 canvas
...
EXPRS = {
    ...
    'tired':      dict(bright=0.80, contrast=0.92, tint=(190,150, 90),  crush=0.22),
    'troubled':   dict(bright=0.86, contrast=1.30, tint=(130,130,175),  crush=0.28),
    'silent':     dict(bright=0.60, contrast=1.18, tint=( 70, 70,115),  crush=0.42),
    'thoughtful': dict(bright=1.06, contrast=1.06, tint=(210,185,140),  crush=0.00),
    'reading':    dict(bright=0.90, contrast=1.10, tint=(175,155,115),  crush=0.12),
}

print("Step 3: 渲染 6 個表情...")
